...
- **频率-时间曲线**：每个高电平脉冲生成一个频率点（`freq = 1 / (2 × 脉宽)`），与逻辑分析仪测量口径一致；可切换为**上升沿周期**模式（`freq = 1 / 上升沿间隔`），工具栏一键切换
...
- **频率重建**：两种计算模式，工具栏一键切换——
  - **脉冲宽度**（默认）：逻辑分析仪导出的跳变严格 1/0 交替，每个高电平脉冲由上升沿+下降沿显式界定、脉宽直接可算——频率点横坐标取该脉冲的上升沿时刻、频率 `1/(2×脉宽)`（等价于假设占空比 50%，与逻辑分析仪测量口径一致），同时显示实测占空比；低电平区间不生成频率点，避免阶梯状曲线。勾选「占空比修正」后改用 `1/(2×脉宽) × (占空比/50%) = 1/周期`，适合窄脉冲/占空比变化信号；周期与占空比默认按相邻两脉冲**下降沿**间隔计算，可经「基准」按钮切换为上升沿。首脉冲因起始段不完整，固定按 50% 占空比口径计算
  - **上升沿周期**：相邻两个上升沿的间隔 `dt` 为一个周期，频率 `1/dt`，频率点按周期起点上升沿时刻绘制，适合占空比变化或窄脉冲信号
  - 所有格式的跳变均为严格 1/0 交替的方波，边沿时间即数据本身：脉冲宽度与上升沿周期两种模式均直接按边沿对计算（脉宽 = 上升沿→下降沿，周期 = 相邻上升沿间隔），不做任何间隙过滤，扫频末端低速脉冲不会被误删；加减速分段按间隔分布 ≥30 倍"断层"识别真实停歇（如两段测试间的空闲）并切块
