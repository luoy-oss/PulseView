...
AB 相模式会读取 VCD 头中的全部单比特信号定义，例如 `$var wire 1 ! D0 $end` 与 `$var wire 1 " D2 $end`。进入分析页面后必须在下拉框中按实际接线手动选择 A 相和 B 相，工具将按四相状态序列分析方向（`00 → 01 → 11 → 10 → 00` 为正向、反序为反向），输出正向正频率、反向负频率的频率-时间曲线，并给出 A/B 脉冲数、边沿数、正反向周期数、平均周期、相位差及非法跳变数。解析兼容一个时间戳行中包含多路变化的 PulseView VCD 写法，例如 `#0 0! 0"`。
